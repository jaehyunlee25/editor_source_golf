...
function mkScreen() {
  ...
  elSplit.val = param.splitterDate;
  elSplit.onclick = divclick;
  ...
  if (gg && gg.func && gg.func == "dict") elDict.onclick();
  if (gg && gg.func && gg.func == "dictCourse") elDC.onclick();
  if (gg && gg.func && gg.func == "splitterDate") elSplit.onclick();
...
function opReserveScript(club, script) {
  ...
  const splitterDate = getSplitterDate(code);
  const param = { club, dict, funcs, dictCourse, splitterDate };
  btnSave.param = param;
  mkScreen();
}
...
btnSave.onclick = function () {
  ...
  post(urlHeader + "/setReserveReserve", param, httpHeader, (data) => {
...
selClub.onchange = function () {
  ...
  post(urlHeader + "/reservebot_admin", param, httpHeader, (data) => {
...
elMqtt.onclick = function () {
  const param = {
    ...
    command: "reserve", // reserve, reserveSearch, reserveCancel, search
  };
  /* socket.publish(
