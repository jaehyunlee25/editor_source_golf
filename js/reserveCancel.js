...
btnSave.onclick = function () {
  ...
  post(urlHeader + "/setReserveCancel", param, httpHeader, (data) => {
...
selClub.onchange = function () {
  ...
  post(urlHeader + "/reserveCancelbot_admin", param, httpHeader, (data) => {
...
elMqtt.onclick = function () {
  const param = {
    ...
    course: selCourse.value,
    time: [selHour.value, iptMin.value].join(""),
    command: "reserveCancel", // reserve, reserveSearch, reserveCancel, search
  };
  socket.send(
    JSON.stringify({
      command: "publish",
